...
class OrderSumExcelExport implements FromView, WithColumnWidths, WithEvents
{
    ...
    public static function afterSheet(AfterSheet $event)
    {
        ...
        for($i=1;$i<=$lastRow;$i++){
                $active_sheet->getRowDimension($i)->setRowHeight(40);
        }

        foreach(range('C','Z') as $columnID)
        {
            $active_sheet->getColumnDimension($columnID)->setWidth(40);
        }
        $columns = array();
        for ($i = 'A'; $i != 'AV'; $i++) {
            $columns[] = $i;
        ...
        foreach ($columns as $columnID) {
            if (strlen($columnID) > 1 && $columnID >= 'AA' && $columnID <= 'AZ') {
                $active_sheet->getColumnDimension($columnID)->setWidth(80);
            }
        }
